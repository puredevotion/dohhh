...
> **PARTIAL. Sections 1–8 are being written and are not in this file yet.**
>
> The synthesis agent that produced this split its output across turns, and only its
> final message was captured, so sections 1–7 were lost before the adversarial pass
> ever saw them. What follows is sections 8 (tail) through 12, which were written,
> attacked and revised in full. The four research digests behind them are intact, so
> nothing is lost — the missing sections are being regenerated from the same source.
>
> Do not treat the absence of a ruling here as the absence of a ruling.
